...
def run(options)
  ...
  options[:node] = "-name mate@127.0.0.1" unless options[:node]
  ...
  unless tab
    ...
    shell_command = "echo -n -e \"\\033]0;#{tab_name}\\007\"; cd #{source_path}; clear; erl -pa . -pa ../ebin -pl ../deps/*/ebin #{options[:node]} "
